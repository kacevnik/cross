    ...
    function postSolution(id){
        ...
        var select_h = parseInt(document.getElementById('select_h').value.trim());
        var select_m = parseInt(document.getElementById('select_m').value.trim());
        var cal_date = document.getElementById('cal_date').value.trim();
        var show_date = (document.getElementById('show_date').checked)?1:0;
        var nabor = document.querySelectorAll('.cma');
        var cma = '';
        var top = [];
        ...
        $.post( "../inc/add_report_constructor.php", {solution: cma, top: topNew, left: topLeft, name: name, h: h, w: w, cross:id, pass:pass, show_date:show_date, select_h:select_h, select_m:select_m, cal_date:cal_date},function(data){
